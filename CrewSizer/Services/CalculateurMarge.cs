...
public static class CalculateurMarge
{
    public static ResultatMarge Calculer(Configuration config)
    {
        ...
        {
            Mois = config.Periode.Mois,
            Annee = config.Periode.Annee,
        ...
        // 6. Allocation cabine : préférence 1 CC + 1 PNC, sinon 2 CC
        double pncUsed = Math.Min(pncCap, rotations);
        int ccNeed = rotations + (int)Math.Ceiling(rotations - pncUsed);
        int pncNeed = (int)Math.Floor(pncUsed);

        result.RotationsAvecPNC = pncNeed;
        result.RotationsSansPNC = rotations - pncNeed;

        // 7. Marges par sous-catégorie
        result.CDB = CalculerCategorie("CDB", config.Effectif.Cdb, cdbCap, rotations);
        result.OPL = CalculerCategorie("OPL", config.Effectif.Opl, oplCap, rotations);
        result.CC = CalculerCategorie("CC", config.Effectif.Cc, ccCap, ccNeed);
        result.PNCDetail = CalculerCategorie("PNC", config.Effectif.Pnc, pncCap, pncNeed);
    ...
    private static ResultatCategorie CalculerCategorie(string nom, int effectif, double capacite, int besoin)
    {
        double tau = capacite > 0
            ? besoin / capacite
            : (besoin > 0 ? double.PositiveInfinity : 0);
